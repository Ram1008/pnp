const productData = [
  {
    id: 1,
    category: "Visiting Cards",
    image: "/categories/visiting-cards.png",
    features: [
      "Custom printing available",
      "Premium quality",
      "Fast delivery",
      "Bulk discounts",
      "Satisfaction guaranteed",
    ],
    pricing: [
      {
        qty: 50,
        price: 1500,
        unit: 30,
        isDefault: true,
      },
      {
        qty: 100,
        price: 2500,
        unit: 25,
        savings: 17,
        recommended: true,
      },
      {
        qty: 200,
        price: 4500,
        unit: 22.5,
        savings: 25,
        },
      {
        qty: 300,
        price: 6500,
        unit: 20,
        savings: 30,
        },
      {
        qty: 400,
        price: 8500,
        unit: 17.5,
        savings: 35,
        },
      {
        qty: 500,
        price: 10500,
        unit: 15,
        savings: 40,
        },
      {
        qty: 600,
        price: 12500,
        unit: 12.5,
        savings: 45,
        },
      {
        qty: 700,
        price: 14500,
        unit: 10,
        savings: 50,
        },
      {
        qty: 800,
        price: 16500,
        unit: 7.5,
        savings: 55,
        },
      {
        qty: 900,
        price: 18500,
        unit: 5,
        savings: 60,
        },
      {
        qty: 1000,
        price: 20500,
        unit: 2.5,
        savings: 65,
        },
    ],
    freeShipping: "Free shipping on orders in Visiting Cards",
    items: [
      {
        id: 1,
        name: "Standard Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 300,
          minQty: 50,
        },
      },
      {
        id: 2,
        name: "Classic Visiting Card",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 310,
          minQty: 50,
        },
      },
      {
        id: 3,
        name: "Rounded Corner Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 320,
          minQty: 50,
        },
      },
      {
        id: 4,
        name: "Squaare Visiting cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 330,
          minQty: 50,
        },
      },
      {
        id: 5,
        name: "Leaf Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 340,
          minQty: 50,
        },
      },
      {
        id: 6,
        name: "Oval Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 350,
          minQty: 50,
        },
      },
      {
        id: 7,
        name: "Circular Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 360,
          minQty: 50,
        },
      },
      {
        id: 8,
        name: "Spot UV Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 370,
          minQty: 50,
        },
      },
      {
        id: 9,
        name: "Raised Foil Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 380,
          minQty: 50,
        },
      },
      {
        id: 10,
        name: "Glossy Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 390,
          minQty: 50,
        },
      },
      {
        id: 11,
        name: "Matte Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 400,
          minQty: 50,
        },
      },
      {
        id: 12,
        name: "Magnetic Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 410,
          minQty: 50,
        },
      },
      {
        id: 13,
        name: "Transparent Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 420,
          minQty: 50,
        },
      },
      {
        id: 14,
        name: "Premium Plus Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 430,
          minQty: 50,
        },
      },
      {
        id: 15,
        name: "Non-Tearavle Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 440,
          minQty: 50,
        },
      },
      {
        id: 16,
        name: "Velvet Touch Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 450,
          minQty: 50,
        },
      },
      {
        id: 17,
        name: "Pearl Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 460,
          minQty: 50,
        },
      },
      {
        id: 18,
        name: "Craft Visiting Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 470,
          minQty: 50,
        },
      },
    ],
  },
  {
    id: 2,
    category: "Labels, Stickers & Packaging",
    features: [
      "Custom printing available",
      "Premium quality",
      "Fast delivery",
      "Bulk discounts",
      "Satisfaction guaranteed",
    ],
    pricing: [
      {
        qty: 50,
        price: 1500,
        unit: 30,
        isDefault: true,
      },
      {
        qty: 100,
        price: 2500,
        unit: 25,
        savings: 17,
        recommended: true,
      },
      {
        qty: 200,
        price: 4500,
        unit: 22.5,
        savings: 25,
      },
    ],
    freeShipping: "Free shipping on orders in Labels, Stickers & Packaging",
    items: [
      {
        id: 1,
        name: "Packaging Sleeves",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 300,
          minQty: 50,
        },
      },
      {
        id: 2,
        name: "Corrugated Boxes",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 310,
          minQty: 50,
        },
      },
      {
        id: 3,
        name: "Flat Mailer Boxes",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 320,
          minQty: 50,
        },
      },
      {
        id: 4,
        name: "multipurpose Boxes",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 330,
          minQty: 50,
        },
      },
      {
        id: 5,
        name: "Self Adhesive Tapes",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 340,
          minQty: 50,
        },
      },
      {
        id: 6,
        name: "Kraft Paper Stand Up Pouches",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 350,
          minQty: 50,
        },
      },
      {
        id: 7,
        name: "Product Boxes",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 360,
          minQty: 50,
        },
      },
      {
        id: 8,
        name: "Wrapping Papers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 370,
          minQty: 50,
        },
      },
      {
        id: 9,
        name: "Courier Bags",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 380,
          minQty: 50,
        },
      },
      {
        id: 10,
        name: "Sheet Stickers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 390,
          minQty: 50,
        },
      },
      {
        id: 11,
        name: "Visiting Card Stickers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 400,
          minQty: 50,
        },
      },
      {
        id: 12,
        name: "Custom Car Stickers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 410,
          minQty: 50,
        },
      },
      {
        id: 13,
        name: "Custom Floor Stickers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 420,
          minQty: 50,
        },
      },
      {
        id: 14,
        name: "Sticker Singles",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 430,
          minQty: 50,
        },
      },
      {
        id: 15,
        name: "Floor Stickers-Social Distance",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 440,
          minQty: 50,
        },
      },
      {
        id: 16,
        name: "Window Stickers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 450,
          minQty: 50,
        },
      },
      {
        id: 17,
        name: "Foil Stickers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 460,
          minQty: 50,
        },
      },
      {
        id: 18,
        name: "Shipping and Mailing Labels",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 470,
          minQty: 50,
        },
      },
      {
        id: 19,
        name: "Product & Packaging Labels",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 480,
          minQty: 50,
        },
      },
      {
        id: 20,
        name: "Return Address Labels",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 490,
          minQty: 50,
        },
      },
      {
        id: 21,
        name: "Transparent Labels",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 500,
          minQty: 50,
        },
      },
      {
        id: 22,
        name: "Hang Tags",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 510,
          minQty: 50,
        },
      },
      {
        id: 23,
        name: "Name Tags",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 520,
          minQty: 50,
        },
      },
      {
        id: 24,
        name: "Baggage Tags",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 530,
          minQty: 50,
        },
      },
      {
        id: 25,
        name: "Folded Hang Tags",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 540,
          minQty: 50,
        },
      },
      {
        id: 26,
        name: "UV Ink Transfer Stickers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 550,
          minQty: 50,
        },
      },
      {
        id: 27,
        name: "QR Code Stickers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 560,
          minQty: 50,
        },
      },
      {
        id: 28,
        name: "Custom Iron-on Labels",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 570,
          minQty: 50,
        },
      },
      {
        id: 29,
        name: "Lock Bottom Boxes",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 580,
          minQty: 50,
        },
      },
      {
        id: 30,
        name: "Auto Lock Bottom Boxes",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 590,
          minQty: 50,
        },
      },
      {
        id: 31,
        name: "Custom Shape Stickers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 600,
          minQty: 50,
        },
      },
      {
        id: 32,
        name: "Glow in the Dark Stickers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 610,
          minQty: 50,
        },
      },
      {
        id: 33,
        name: "Holographic Stickers",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 620,
          minQty: 50,
        },
      },
    ],
  },
  {
    id: 3,
    category: "Personalised Pens",
    features: [
      "Custom printing available",
      "Premium quality",
      "Fast delivery",
      "Bulk discounts",
      "Satisfaction guaranteed",
    ],
    pricing: [
      {
        qty: 50,
        price: 1500,
        unit: 30,
        isDefault: true,
      },
      {
        qty: 100,
        price: 2500,
        unit: 25,
        savings: 17,
        recommended: true,
      },
      {
        qty: 200,
        price: 4500,
        unit: 22.5,
        savings: 25,
      },
    ],
    freeShipping: "Free shipping on orders in Personalised Pens",
    items: [
      {
        id: 1,
        name: "Personalised Pens",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 300,
          minQty: 50,
        },
      },
      {
        id: 2,
        name: "Customized Pens",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 310,
          minQty: 50,
        },
      },
      {
        id: 3,
        name: "Premium Matte Pens",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 320,
          minQty: 50,
        },
      },
      {
        id: 4,
        name: "Premium Magnetic Metal Roller Pens",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 330,
          minQty: 50,
        },
      },
      {
        id: 5,
        name: "Premium Brush Stone Black Ballpoint",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 340,
          minQty: 50,
        },
      },
      {
        id: 6,
        name: "Black Matte Ball Pens",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 350,
          minQty: 50,
        },
      },
      {
        id: 7,
        name: "Plastic Pens",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 360,
          minQty: 50,
        },
      },
      {
        id: 8,
        name: "Grand Opaque Ballpoint Pen",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 370,
          minQty: 50,
        },
      },
      {
        id: 9,
        name: "Japan Metallic Ballpoint Pen",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 380,
          minQty: 50,
        },
      },
      {
        id: 10,
        name: "new Getz Cap-Type Ballpoint Pens",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 390,
          minQty: 50,
        },
      },
    ],
  },
  {
    id: 4,
    category: "Calendars",
    features: [
      "Custom printing available",
      "Premium quality",
      "Fast delivery",
      "Bulk discounts",
      "Satisfaction guaranteed",
    ],
    pricing: [
      {
        qty: 50,
        price: 1500,
        unit: 30,
        isDefault: true,
      },
      {
        qty: 100,
        price: 2500,
        unit: 25,
        savings: 17,
        recommended: true,
      },
      {
        qty: 200,
        price: 4500,
        unit: 22.5,
        savings: 25,
      },
    ],
    freeShipping: "Free shipping on orders in Calendars",
    items: [
      {
        id: 1,
        name: "Desk Calendars",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 300,
          minQty: 50,
        },
      },
      {
        id: 2,
        name: "Wall Calendars",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 310,
          minQty: 50,
        },
      },
      {
        id: 3,
        name: "Custom Base Spiral Desk Calendars",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 320,
          minQty: 50,
        },
      },
      {
        id: 4,
        name: "Wall Calendar- New Size",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 330,
          minQty: 50,
        },
      },
      {
        id: 5,
        name: "Magnet Calendars",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 340,
          minQty: 50,
        },
      },
      {
        id: 6,
        name: "Flip Desk Calendars",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 350,
          minQty: 50,
        },
      },
      {
        id: 7,
        name: "Poster Calendars",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 360,
          minQty: 50,
        },
      },
      {
        id: 8,
        name: "Pocket Calendars",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 370,
          minQty: 50,
        },
      },
      {
        id: 9,
        name: "Desk Calendars with Photo Frames",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 380,
          minQty: 50,
        },
      },
      {
        id: 10,
        name: "Four Sheeter Wall Calendars",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 390,
          minQty: 50,
        },
      },
      {
        id: 11,
        name: "Perpetual Calendars",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 400,
          minQty: 50,
        },
      },
      {
        id: 12,
        name: "Premium Themed Wall Calendars",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 410,
          minQty: 50,
        },
      },
      {
        id: 13,
        name: "Shree Ganesha Wall Calendars",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 420,
          minQty: 50,
        },
      },
    ],
  },
  {
    id: 5,
    category: "Invitations & Announcements",
    features: [
      "Custom printing available",
      "Premium quality",
      "Fast delivery",
      "Bulk discounts",
      "Satisfaction guaranteed",
    ],
    pricing: [
      {
        qty: 50,
        price: 1500,
        unit: 30,
        isDefault: true,
      },
      {
        qty: 100,
        price: 2500,
        unit: 25,
        savings: 17,
        recommended: true,
      },
      {
        qty: 200,
        price: 4500,
        unit: 22.5,
        savings: 25,
        },
        {
        qty: 300,
        price: 6500,
        unit: 20,
        savings: 25,
        }, {
        qty: 400,
        price: 8500,
        unit: 17.5,
        savings: 25,
        }, {
        qty: 500,
        price: 10500,
        unit: 15,
        savings: 25,
        },
        {
        qty: 600,
        price: 12500,
        unit: 12.5,
        savings: 25,
        },
        {
        qty: 700,
        price: 14500,
        unit: 10,
        savings: 25,
        },
        {
        qty: 800,
        price: 16500,
        unit: 7.5,
        savings: 25,
        },
        {
        qty: 900,
        price: 18500,
        unit: 5,
        savings: 25,
        },
        {
        qty: 1000,
        price: 20500,
        unit: 2.5,
        savings: 25,
        },
    ],
    freeShipping: "Free shipping on orders in Invitations & Announcements",
    items: [
      {
        id: 1,
        name: "Birthaday Invitation Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 300,
          minQty: 50,
        },
      },
      {
        id: 2,
        name: "Party Invitation",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 310,
          minQty: 50,
        },
      },
      {
        id: 3,
        name: "Save the Date Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 320,
          minQty: 50,
        },
      },
      {
        id: 4,
        name: "Baby shower Invitations",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 330,
          minQty: 50,
        },
      },
      {
        id: 5,
        name: "Thank You Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 340,
          minQty: 50,
        },
      },
      {
        id: 6,
        name: "Religious Announcements",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 350,
          minQty: 50,
        },
      },
      {
        id: 7,
        name: "Graduation Invitation",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 360,
          minQty: 50,
        },
      },
      {
        id: 8,
        name: "Envelopes",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 370,
          minQty: 50,
        },
      },
      {
        id: 9,
        name: "Business Invitations",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 380,
          minQty: 50,
        },
      },
      {
        id: 10,
        name: "Note Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 390,
          minQty: 50,
        },
      },
    ],
  },
  {
    id: 6,
    category: "Weddings",
    features: [
      "Custom printing available",
      "Premium quality",
      "Fast delivery",
      "Bulk discounts",
      "Satisfaction guaranteed",
    ],
    pricing: [
      {
        qty: 50,
        price: 1500,
        unit: 30,
        isDefault: true,
      },
      {
        qty: 100,
        price: 2500,
        unit: 25,
        savings: 17,
        recommended: true,
      },
      {
        qty: 200,
        price: 4500,
        unit: 22.5,
        savings: 25,
      },
    ],
    freeShipping: "Free shipping on orders in Weddings",
    items: [
      {
        id: 1,
        name: "Wedding Invitations",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 300,
          minQty: 50,
        },
      },
      {
        id: 2,
        name: "RSVP Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 310,
          minQty: 50,
        },
      },
      {
        id: 3,
        name: "Save the Dates",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 320,
          minQty: 50,
        },
      },
      {
        id: 4,
        name: "Thank You Cards",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 330,
          minQty: 50,
        },
      },
      {
        id: 5,
        name: "Gold Foil Wedding Invitation",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 340,
          minQty: 50,
        },
      },
      {
        id: 6,
        name: "Wedding Enclosure Card",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 350,
          minQty: 50,
        },
      },
      {
        id: 7,
        name: "Wedding Programmes",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 360,
          minQty: 50,
        },
      },
      {
        id: 8,
        name: "Wedding Menus",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 370,
          minQty: 50,
        },
      },
      {
        id: 9,
        name: "Rehearsal Dinner Invitations",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 380,
          minQty: 50,
        },
      },
      {
        id: 10,
        name: "Bridal Shower invitations",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 390,
          minQty: 50,
        },
      },
    ],
  },
  {
    id: 7,
    category: "Passport Size Photos",
    features: [
      "Custom printing available",
      "Premium quality",
      "Fast delivery",
      "Bulk discounts",
      "Satisfaction guaranteed",
    ],
    pricing: [
      {
        qty: 50,
        price: 1500,
        unit: 30,
        isDefault: true,
      },
      {
        qty: 100,
        price: 2500,
        unit: 25,
        savings: 17,
        recommended: true,
      },
      {
        qty: 200,
        price: 4500,
        unit: 22.5,
        savings: 25,
      },
    ],
    freeShipping: "Free shipping on orders in Passport Size Photos",
    items: [
      {
        id: 1,
        name: "Passport Size Photographs",
        features: ["Feature A", "Feature B", "Feature C"],
        pricing: {
          basePrice: 300,
          minQty: 50,
        },
      },
    ],
  },
];

export default productData;
